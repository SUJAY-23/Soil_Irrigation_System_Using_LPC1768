...
int main() 
{
    LPC_GPIO1->FIODIR |= (0x01 << 19);    // Configure P1.19 as output
	LPC_GPIO1->FIOCLR |= (0xFF << 19);    // Configure P1.19 as output
    ...
    while (1) 
	{
        ...
        LPC_GPIO1->FIOSET |= (0x01 << 19);    // Set P1.19 high
        delay(100);
        LPC_GPIO1->FIOCLR |= (0x01 << 19);    // Clear P1.19 low
        delay(100);
    }
}
...
void timer_config(void) 
{
    ...
    LPC_TIM0->MR0 = 1000000;      // Set match register for 10 seconds delay
...
void sensors(void)
{
    ...
    sprintf(stemp,"Temp = %0.2f 'C",temperature_reading);
    lcd_str_write(stemp);
    uart_str(stemp);
	...
	else if(temp_limit < temperature_reading && moist_limit < moisture)
    {
        uart_str("RELAY OFF_1");
        uart_str("\r\n");
		lcd_cmd_write(0x01);
	    lcd_cmd_write(0x80);
	    lcd_str_write("RELAY IS OFF_1");
		delay(100);
		lcd_cmd_write(0x01);
    }
    ...
    {
        uart_str("RELAY ON 5 Sec");
        uart_str("\r\n");
		lcd_cmd_write(0x01);
	    lcd_cmd_write(0x80);
	    lcd_str_write("RELAY IS ON_2");
		LPC_GPIO1->FIOSET |= relay_pin;
        delay(5000);		   //10 sec relay ON
		lcd_cmd_write(0x01);
		LPC_GPIO1->FIOCLR |= relay_pin;
		delay(100);
    }
    ...
    {
        uart_str("RELAY OFF_2");
        uart_str("\r\n");
		lcd_cmd_write(0x01);
	    lcd_cmd_write(0x80);
	    lcd_str_write("RELAY IS OFF_2");
		delay(100);
		lcd_cmd_write(0x01);
    }
